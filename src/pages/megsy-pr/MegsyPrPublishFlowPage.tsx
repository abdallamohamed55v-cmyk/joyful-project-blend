...
export default function MegsyPrPublishFlowPage({ onClose }: { onClose?: () => void } = {}) {
  ...
  const goto = (s: Step) => {
    const base = `/megsy-pr/${projectId}/publish`;
  ...
  const goBack = () => {
    if (step === "url") {
      if (onClose) onClose();
      else navigate(`/megsy-pr/${projectId}/preview`);
    }
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-32">
        {step === "url" && (
          <UrlStep
            ...
            isPremium={!!isPremium}
            onAddDomain={() => projectId && navigate(`/megsy-pr/${projectId}/domains`)}
          />
        )}
        {step === "audience" && (
        ...
        {step === "done" && (
          <DoneStep
            ...
            onBack={() => navigate(`/megsy-pr/${projectId}/preview`)}
          />
        )}
      </div>
