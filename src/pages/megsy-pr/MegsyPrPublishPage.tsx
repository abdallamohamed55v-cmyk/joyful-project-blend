...
export default function MegsyPrPublishPage({ onClose, onPublished }: Props = {}) {
  ...
  const close = () => {
    if (onClose) {
      ...
      navigate(`/megsy-pr/${projectId}/preview`);
    }
  };
  ...
  const publish = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.url) throw new Error(data.error || "فشل النشر");
        ...
        <div className="flex-1 px-4 py-5 overflow-y-auto overflow-x-hidden">
          ...
          {step === "url" && (
            <>
              ...
              <button
                onClick={() => { close(); setTimeout(() => navigate(`/megsy-pr/${projectId}/domains`), 220); }}
                ...
                <div className="flex items-center justify-between rounded-xl bg-foreground/[0.04] p-2 pe-1.5">
                  ...
                  <button
                    onClick={() => {
                      ...
                      toast.success("تم إنشاء صورة المشاركة");
                    }}
          ...
          {step === "review" && (
            <>
              ...
              <button
                onClick={() => { close(); setTimeout(() => navigate(`/megsy-pr/${projectId}/security`), 220); }}
          ...
          {step === "publishing" && (
            <div className="pt-12 pb-10 grid place-items-center text-center">
              ...
              <div className="text-lg font-semibold mb-1">جاري النشر…</div>
              <div className="text-sm text-muted-foreground">قد يستغرق هذا بضع لحظات</div>
            </div>
          )}
          ...
          {step === "done" && (
            <div className="pt-6 pb-2 grid place-items-center text-center">
              ...
              <div className="w-full rounded-2xl border border-foreground/10 bg-background p-3 flex items-center gap-2 mb-5 min-w-0">
                ...
                <button
                  onClick={() => { navigator.clipboard.writeText(publishedUrl || `https://${fullUrl}`); toast.success("تم النسخ"); }}
