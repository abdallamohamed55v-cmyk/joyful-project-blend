...
export default function WorkspaceCreatePage() {
  ...
  const create = async (selectedPlan: string | null) => {
    ...
    toast.success("تم إنشاء الورك سبيس");
  ...
  const handleContinueWithPlan = async () => {
    ...
    if (!isWorkspacePaidPlan(plan)) {
      toast.error("الباقة غير مدعومة حالياً");
      return;
    }
      ...
      <header className="sticky top-0 z-10 bg-background/95 backdrop-blur border-b border-border/40">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold">
            {step === "name" ? "ورك سبيس جديدة" : "اختر الباقة"}
          </h1>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        {step === "name" && (
          <div className="space-y-6">
            <div className="space-y-2">
              <h2 className="text-2xl font-semibold tracking-tight">ما اسم الورك سبيس؟</h2>
              <p className="text-sm text-muted-foreground">يمكنك تغييره لاحقاً من الإعدادات.</p>
            </div>
            <Input
        ...
        {step === "plan" && (
          <div className="space-y-5">
            <div className="space-y-2">
              <h2 className="text-2xl font-semibold tracking-tight">اختر باقة لـ {name}</h2>
              <p className="text-sm text-muted-foreground">يمكنك تخطي هذه الخطوة وتغييرها لاحقاً.</p>
            </div>
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {p.monthlyPrice === 0 ? "$0" : `$${p.monthlyPrice}/شهر`}
                        </span>
                      </div>
            ...
            <div className="flex gap-2 pt-2">
              ...
              >
                {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : plan === "free" ? "إنشاء" : "الدفع والمتابعة"}
              </Button>
            </div>
          </div>
        )}
