...
import heroImg from "@/assets/tool-landing/video-extender-v8.jpg";
import samplePoster from "@/assets/video-extender-sample-boy-v2.jpg";
  ...
  const handleGenerate = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("video-tools", {
        body: { tool: "video-extender", model: FAL_MODEL, video, extraSeconds, withAudio },
      });
              ...
              <div className="flex-1 overflow-y-auto px-4 pt-[calc(env(safe-area-inset-top)+4rem)] pb-[180px]">
                ...
                <div className="max-w-sm mx-auto">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { v: false, label: "No Audio", price: "3 MC/s" },
                      { v: true, label: "With Audio", price: "5 MC/s" },
                    ].map((opt) => {
                      const active = withAudio === opt.v;
                      return (
                        <button
                          key={opt.label}
                          onClick={() => setWithAudio(opt.v)}
                        ...
                        >
                          <div>{opt.label}</div>
                          <div className="text-[10px] font-normal text-white/60 mt-0.5">{opt.price}</div>
                        </button>
                      );
                    })}
                  </div>
