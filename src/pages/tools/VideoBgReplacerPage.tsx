...
import { ArrowLeft, Upload, Download, Share2, RefreshCw, Film, ImagePlus } from "lucide-react";
...
// Fal model dedicated to video background replacement
const FAL_MODEL = "fal-ai/video-background-replacement";
...
const VideoBgReplacerPage = () => {
  ...
  const readImage = (file: File) => {
    ...
    const r = new FileReader();
    r.onload = (e) => setBgImage(e.target?.result as string);
    r.readAsDataURL(file);
  };

  const handleGenerate = async () => {
    if (!video) { toast.error("Upload a video"); return; }
    if (mode === "image" && !bgImage) { toast.error("Upload a background"); return; }
    if (mode === "prompt" && !prompt.trim()) { toast.error("Describe the background"); return; }
    ...
    try {
      const { data, error } = await supabase.functions.invoke("video-tools", {
        body: {
          tool: "video-bg-replacer",
          model: FAL_MODEL,
          video,
          backgroundImage: mode === "image" ? bgImage : undefined,
  ...
  const handleShare = () => {
    if (!resultUrl) return;
    if (navigator.share) navigator.share({ url: resultUrl, title: "New background" }).catch(() => {});
  ...
  return (
    <div className="h-[100dvh] flex flex-col overflow-hidden relative bg-gradient-to-b from-zinc-950 via-black to-zinc-950">
      <div className="pointer-events-none absolute -top-40 -left-32 w-[480px] h-[480px] rounded-full bg-violet-500/25 blur-[120px]" />
      <div className="pointer-events-none absolute top-1/2 -right-40 w-[520px] h-[520px] rounded-full bg-sky-500/25 blur-[140px]" />
      <div className="pointer-events-none absolute bottom-0 left-1/4 w-[420px] h-[420px] rounded-full bg-emerald-400/15 blur-[120px]" />
        ...
        <AnimatePresence mode="wait">
          {stage === "landing" && (
            ...
            >
              ...
              >
                <img src={heroImg} alt="Video BG Replacer" className="absolute inset-0 w-full h-full object-cover" />
              ...
              >
                <h2 className="font-display text-[10vw] sm:text-[6vw] uppercase leading-[0.95] tracking-tight text-white">
                  New <span className="bg-gradient-to-r from-violet-300 via-sky-300 to-emerald-200 bg-clip-text text-transparent">backdrop.</span>
                </h2>
                <p className="mt-2 text-sm text-white/65">
                  Upload a clip — drop yourself anywhere with one tap.
                </p>
              </motion.div>
            ...
            >
              <div className="flex-1 overflow-y-auto px-4 pt-[calc(env(safe-area-inset-top)+4rem)] pb-[180px]">
                <div className="max-w-sm mx-auto mb-5">
                  <div className={`relative rounded-[28px] overflow-hidden aspect-[4/5] ${GLASS}`}>
                    ...
                    {video ? (
                      <video src={video} className="absolute inset-0 w-full h-full object-cover mix-blend-screen opacity-95" muted playsInline loop autoPlay />
                    ) : (
                ...
                <div className="max-w-sm mx-auto">
                  <h3 className="mb-2 text-[11px] font-bold tracking-[0.18em] text-white/60 uppercase">Background Source</h3>
                  <div className="grid grid-cols-2 gap-2 mb-5">
                    {(["image", "prompt"] as Mode[]).map((m) => {
                      const active = mode === m;
                      return (
                        <button
                          key={m}
                          onClick={() => setMode(m)}
                          className={`px-2 py-2.5 rounded-2xl text-xs font-semibold text-white capitalize transition ${
                            active
                              ? "bg-white/20 backdrop-blur-2xl border border-white/30 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.4)]"
                              : "bg-white/5 backdrop-blur-2xl border border-white/10"
                          }`}
                        >
                          {m === "image" ? "Image" : "AI Prompt"}
                        </button>
                      );
                    })}
                  </div>
                  ...
                  {mode === "image" ? (
                    <>
                      ...
                      <button
                        ...
                        className={`w-full px-3 py-3 rounded-2xl flex items-center gap-2 text-white text-xs font-semibold ${GLASS_PILL}`}
                      >
                        {bgImage ? (
                          <>
                            <img src={bgImage} alt="" className="w-7 h-7 rounded-lg object-cover" />
                            <span>Background ready · tap to change</span>
                          </>
                        ) : (
                          <>
                    ...
                    <input
                      ...
                      placeholder="Sunset beach, cinematic…"
                      className={`w-full px-4 py-3 rounded-2xl text-sm text-white placeholder:text-white/40 focus:outline-none ${GLASS_PILL}`}
                    />
                  )}
                </div>
              </div>
              ...
              >
                <motion.button
                  ...
                  disabled={!video || (mode === "image" ? !bgImage : !prompt.trim())}
