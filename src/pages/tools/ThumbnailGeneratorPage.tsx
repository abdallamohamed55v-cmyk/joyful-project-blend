  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("image-tools", {
        body: { tool: "logo-generator", model: FAL_MODEL, prompt, referenceImage: refImage },
      });
            ...
            >
              <div className="flex-1 overflow-y-auto px-4 pt-[calc(env(safe-area-inset-top)+4rem)] pb-[180px]">
                <div className="max-w-sm mx-auto mb-5">
                  <div className={`relative rounded-[28px] overflow-hidden aspect-[16/10] ${GLASS}`}>
                    ...
                    <button
                      ...
                      className={`absolute top-2 right-2 w-8 h-8 flex items-center justify-center rounded-full text-white ${GLASS_PILL}`}
                    >
                ...
                <div className="max-w-sm mx-auto">
                  ...
                  <h3 className="mb-2 text-[11px] font-bold tracking-[0.18em] text-white/60 uppercase">Reference (optional)</h3>
                  <button
                    onClick={() => imageInputRef.current?.click()}
                  ...
                  >
                    {refImage ? (
                      <>
                        <img src={refImage} alt="" className="w-7 h-7 rounded-lg object-cover" />
                        <span>Reference loaded · tap to change</span>
                      </>
                    ) : (
                      <>
