...
// Fal model dedicated to video matting / green-screen removal
const FAL_MODEL = "fal-ai/ben/v2/video";
...
const GreenScreenPage = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("video-tools", {
        body: { tool: "green-screen", model: FAL_MODEL, video, background: bg },
      });
  ...
  const handleShare = () => {
    if (!resultUrl) return;
    if (navigator.share) navigator.share({ url: resultUrl, title: "Background removed" }).catch(() => {});
  ...
  return (
    <div className="h-[100dvh] flex flex-col overflow-hidden relative bg-gradient-to-b from-zinc-950 via-black to-zinc-950">
      <div className="pointer-events-none absolute -top-40 -left-32 w-[480px] h-[480px] rounded-full bg-emerald-500/30 blur-[120px]" />
      <div className="pointer-events-none absolute top-1/2 -right-40 w-[520px] h-[520px] rounded-full bg-teal-500/25 blur-[140px]" />
      <div className="pointer-events-none absolute bottom-0 left-1/4 w-[420px] h-[420px] rounded-full bg-lime-400/15 blur-[120px]" />
        ...
        <AnimatePresence mode="wait">
          {stage === "landing" && (
            ...
            >
              ...
              >
                <img src={heroImg} alt="Green Screen Remover" className="absolute inset-0 w-full h-full object-cover" />
              ...
              >
                <h2 className="font-display text-[10vw] sm:text-[6vw] uppercase leading-[0.95] tracking-tight text-white">
                  Remove <span className="bg-gradient-to-r from-emerald-300 via-teal-300 to-lime-200 bg-clip-text text-transparent">the green.</span>
                </h2>
                <p className="mt-2 text-sm text-white/65">
                  Upload a green-screen clip — clean alpha matte in seconds.
                </p>
              </motion.div>
            ...
            >
              <div className="flex-1 overflow-y-auto px-4 pt-[calc(env(safe-area-inset-top)+4rem)] pb-[180px]">
                ...
                <div className="max-w-sm mx-auto">
                  <h3 className="mb-2 text-[11px] font-bold tracking-[0.18em] text-white/60 uppercase">Background</h3>
                  <div className="grid grid-cols-4 gap-2">
                    {BG_MODES.map((m) => {
                      const active = bg === m.id;
                      return (
                        <button
                          key={m.id}
                          onClick={() => setBg(m.id)}
          ...
          {stage === "result" && resultUrl && (
            ...
            >
              <div
                className={`relative rounded-[28px] overflow-hidden mx-auto w-full max-w-sm flex-1 max-h-[65vh] ${GLASS}`}
                style={{
                  backgroundImage:
                    bg === "transparent"
                      ? "linear-gradient(45deg,#333 25%,transparent 25%),linear-gradient(-45deg,#333 25%,transparent 25%),linear-gradient(45deg,transparent 75%,#333 75%),linear-gradient(-45deg,transparent 75%,#333 75%)"
                      : undefined,
                  backgroundSize: bg === "transparent" ? "20px 20px" : undefined,
                  backgroundPosition: bg === "transparent" ? "0 0,0 10px,10px -10px,-10px 0" : undefined,
                }}
              >
                <video src={resultUrl} controls autoPlay className="absolute inset-0 w-full h-full object-contain" />
              </div>
