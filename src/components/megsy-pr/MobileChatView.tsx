        ...
        {messages.map((m, i) => {
                ...
                <div className="rounded-2xl border border-foreground/10 px-4 py-3.5 bg-foreground/[0.025]">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-[15px] font-semibold">سأبدأ التنفيذ الآن</div>
                      <div className="text-[12px] text-muted-foreground truncate" dir="auto">
                        {shortPrompt || "هتابع التعديلات خطوة بخطوة"}
                      </div>
                    </div>
                  </div>
                  <div className="mt-3 flex flex-col gap-2 text-[13px]">
                    ...
                    {!hasAnyStep && (
                      <div className="text-muted-foreground ai-shimmer px-1">أجهّز خطة التنفيذ…</div>
                    )}
                  </div>
                </div>
          ...
          return (
            <div key={id} className="mt-4">
              ...
              {/* Done activity card */}
              <div
                className="rounded-2xl border border-foreground/10 px-4 py-3.5 mb-3 backdrop-blur-2xl backdrop-saturate-150 shadow-[0_8px_24px_-12px_rgba(0,0,0,0.18)] dark:shadow-[0_8px_24px_-12px_rgba(0,0,0,0.6)]"
                style={{ backgroundColor: "color-mix(in oklab, hsl(var(--background)) 55%, transparent)" }}
              >
                <div className="flex items-center gap-2 mb-1">
                  <div className="flex-1 min-w-0 font-semibold text-[15px] truncate" dir="auto">
                    {m.activityTitle || (summary ? `تم: ${summary}` : shortPrompt ? `تم تنفيذ: ${shortPrompt}` : "تم التنفيذ")}
                  </div>
                </div>
                <div className="mb-3" />
                <button
                  onClick={() => navigate(`/build/${projectId}/preview`)}
                  className="w-full h-10 rounded-xl bg-foreground/[0.06] text-[14px] font-medium hover:bg-foreground/[0.1] backdrop-blur-md transition"
                >
                  Preview
                </button>
              </div>
