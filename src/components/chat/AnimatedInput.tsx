...
const AnimatedInput = ({ value, onChange, onSend, onCancel, onPlusClick, disabled, isLoading, placeholders, pendingQuestions, onQuestionAnswer, onQuestionSkip, activeAgent, onAgentSelect, onAgentRemove, mentionCategories, selectedModel, onModelSelect, onModelRemove, accentMode, headerSlot, inlineSlot }: AnimatedInputProps) => {
  ...
  return (
    <div className="relative">
      ...
      <div className="chat-composer-frame pointer-events-auto rounded-[1.35rem] px-4 pt-1 pb-2 relative z-10 bg-background shadow-xl">
        <AnimatePresence>
        ...
        <div className="relative px-1">
          {!value && displayedPlaceholder && (
            <div
              aria-hidden
              className="pointer-events-none absolute inset-0 hidden md:flex items-start px-1 pt-2 text-[15px] text-muted-foreground/50 leading-relaxed overflow-hidden"
            >
              <TypingAnimation key={displayedPlaceholder} duration={55} showCursor={false} className="truncate">
          ...
          <textarea
            ...
            placeholder=""
            rows={1}
            className="relative w-full bg-transparent border-none outline-none resize-none text-[15.5px] text-neutral-900 md:text-foreground py-1.5 px-1 leading-relaxed"
            style={{ minHeight: "38px" }}
          />
        </div>

        {/* Bottom controls row */}
        <div className="relative flex items-center gap-2 pt-1 -ml-3 -mr-1">
          <motion.button
            whileTap={{ scale: 0.92 }}
          ...
          <AnimatePresence mode="popLayout" initial={false}>
            {isLoading ? (
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                onClick={onCancel}
                className="shrink-0 w-9 h-9 flex items-center justify-center rounded-full bg-foreground text-background"
              ...
              <motion.button
                ...
                className="shrink-0 w-9 h-9 flex items-center justify-center rounded-full bg-neutral-100 text-neutral-800 md:bg-foreground/10 md:text-foreground hover:bg-foreground/15 transition-colors disabled:opacity-50"
