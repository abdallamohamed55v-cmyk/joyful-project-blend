...
import { ChatIcon, MediaIcon, CodeIcon } from "@/components/sidebar/SidebarIcons";
...
const AppSidebar = ({
  ...
  const navItems = [
    ...
    { label: "Media", Icon: MediaIcon, path: "/media", match: (p: string) => p.startsWith("/media") || p.startsWith("/images") || p.startsWith("/videos") },
    { label: "Code", Icon: CodeIcon, path: "/build", match: (p: string) => p.startsWith("/build") },
  ];
  ...
  const innerContent = (
    <div
      className="flex flex-col h-full text-foreground relative overflow-hidden bg-white dark:bg-black"
    >
      {/* STICKY TOP — Megsy name + collapse */}
      <div
        className={`relative shrink-0 px-4 pt-4 pb-2 flex items-center ${isCollapsed ? "justify-center" : "justify-between"}`}
      >
        {!isCollapsed && (
        ...
        {inline && (
          <button
            ...
            className="w-9 h-9 grid place-items-center rounded-lg text-muted-foreground hover:text-foreground hover:bg-foreground/5 transition-colors"
      ...
      {/* Account switcher moved to bottom user pill */}

      {/* When COLLAPSED: nav stays fixed (does not scroll) */}
      {isCollapsed && (
        <div className="relative shrink-0 px-3 pb-2 flex flex-col items-center gap-1.5">
      ...
      {/* FLOATING BOTTOM — glass user pill + new chat (always visible) */}
      <div
      ...
      >
        <div className={`pointer-events-auto flex items-center gap-2 ${isCollapsed ? "flex-col" : ""}`}>
          {isCollapsed ? (
            <button
              onClick={() => { navigate("/settings"); onClose(); }}
              className="w-11 h-11 rounded-full grid place-items-center text-[13px] font-semibold overflow-hidden border border-foreground/10"
              style={glassStyle}
              title={displayName}
            >
              {displayAvatar ? <img src={displayAvatar} alt="" className="w-11 h-11 rounded-full object-cover" /> : initial}
            </button>
          ) : (
            <div
          ...
          {/* Floating new chat — same glass surface */}
          <button
